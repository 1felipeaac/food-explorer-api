...
class OrdersService {
    ...
    try {
      for (let order of orders) {
        ...
        for (const result of results) {
          ...
          userOrder.push(
            `${order.amount}x ${result.name} R$ ${result.value.toFixed(2)}`
          );
        }
      }
      ...
      const orderCreated = await this.ordersService.createOrder(
        detailing,
        userId,
        sum
      );
  ...
  async listAllOrders(id, role) {
    ...
    const selectOrdersByUserId = await this.ordersService.selectedOrderByUserId(
      id
    );
  ...
  async update(id, orderStatus) {
    ...
    const updateStatus = await this.ordersService.updateStatusOrder(
      id,
      orderStatus
    );
